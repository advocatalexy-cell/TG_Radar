...
- Гиперссылка ведёт на заголовок поста, а не на название канала: `[Заголовок](URL)`. Заголовок — цельное законченное предложение (не обрывок), пересказывающее суть поста своими словами
- ЗАПРЕЩЕНО обрезать `summary` по числу слов/символов — это обрывает предложение на середине (пример ошибки: `Юристам следует тщательно оценивать риски при загрузке документов клиентов в нейросети из-за` — обрыв перед смысловым концом). Если `summary` длиннее ~12 слов, перепиши его короче целиком, сохранив смысл и грамматическую законченность, а не отрежь хвост
